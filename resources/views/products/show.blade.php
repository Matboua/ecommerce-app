...
@section('content')
    ...
    <section class="w-full flex justify-center p-6">
        <div
            class="w-3/6  p-6 bg-white border border-gray-200 rounded-lg shadow hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700 grid grid-cols-2 gap-4">
            <div class="bg-white flex items-center rounded-md">
                <img class="" src="{{ url($product->image) }}" alt="">
            </div>
            <div>
                <h5 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">Product Detail :
                    {{ $product->name }}</h5>
